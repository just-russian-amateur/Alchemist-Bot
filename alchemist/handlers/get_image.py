...
async def get_photo(
    ...
    async with ChatActionSender.upload_photo(bot=bot, chat_id=message.chat.id):
        ...
        await state.update_data(**{
            RedisKeys.IMAGE: image_for_load,
            RedisKeys.LVL_FILE: lvl_file,
        })
        ...
        try:
            ...
            undef_colors, flasks_id_list = await found_colors_in_flasks(image_for_load)
            await state.update_data(**{
        ...
        if len(undef_colors) == 1:
            while undef_colors[list(undef_colors.keys())[0]] > 0:
                ...
                flasks_id_list = replace_in_list(
                    flasks_id_list,
                    color_id=list(undef_colors.keys())[0]
                )
