...
Я решил сделать данный проект открытым из-за возможных проблем в работе **Telegram** в России, а также потенциальных проблем с монетизацией проекта и, как следствием, ограничением доступности бота для пользователей из России. Данный шаг предпринят для того, чтобы каждый желающий пользователь мог запустить у себя локальную копию бота и использовать его по своему усмотрению, не нарушая **Пользовательское Соглашение** (ссылка на него есть в файле **_all_my_texts.py_**), **законодательство вашей страны** и лицензию **Apache 2.0**, которая выбрана для текущего репозитория. Также, я рассчитываю, что любой заинтересованный пользователь может внести свой вклад в развитие проекта и помочь с исправлением багов (если таковые будут обнаружены) или с улучшением и расширением функционала бота или другими оптимизациями.
...
* **ОС:** Ubuntu 22.04+/Debian 10+, Windows 10/11 + [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) (для базы данных)
* **RAM:** 3+ ГБ
* **SSD:** 15+ ГБ (для VPS-сервера)/10+ ГБ свободного места на локальном ПК или сервере
* **Сеть:** любое стабильное интернет соединение, которое не блокирует запросы и ответы к/от Telegram Bot API (в России на локальных ПК и серверах могут наблюдаться проблемы)
...
Бот написан с использованием библиотеки aiogram 3.x и тестировался на интерпретаторах Python 3.9 и Python 3.13, но должен одинаково работать на всех свежих версиях интерпретатора.
...
* [Интерпретатор Python](https://www.python.org/downloads/) c установленными зависимостями (установленными через PIP):
  ...
  - pytz;
  - dotenv;
  - aiohttp-socks (данный пакет необходим для настройки прокси для бота, если использовать бота на серверах, где запросы к **Telegram** блокируются, примеры использования в ветках с пометками **proxy**);
  - может понадобиться отдельная установка некоторых зависимостей, если они не были скачаны вместе с вышеперечисленными. 
* Redis - NoSQL СУБД с открытым исходным кодом для хранения состояний пользователя в машине состояний и пользовательских данных. Установка и настройка Redis описаны на [официальном сайте (для пользователей из России могут потребоваться дополнительные средства для доступа к сайту)](https://redis.io/docs/latest/get-started/).

> **Примечание:** данный бот использует для своей работы обученную модель на базе [YOLOv11X](https://docs.ultralytics.com/models/yolo11/), которая представлена в данном репозитории, но не могла быть загружена целиком, поскольку превышала допустимый вес в 100 МБ, в связи с чем, чтобы получить данную модель вам нужно установить и настроить не только [Git](https://git-scm.com/install/linux), но и [Git LFS](https://git-lfs.com/) для использования Git для хранения больших файлов.

> **Примечание 2:** в проекте есть ветка **rich-messages-feature**, где представлена обновленная структура сообщений, которая была добавлена в обновлении Telegram Bot API 10.3 для поддержки которой нужна версия библиотеки aiogram 3.31.0 или выше.
...
Структура проекта представляет из себя иерархическую структуру логически отделенных друг от друга функциональных блоков. В корне проекта лежат, по сути, системные файлы и сгенерированное изображение с примерами цветов, которые умеет распознавать бот и пример файла окружения, в котором должен храниться полученный ранее токен (для большей безопасности бота). При использовании реального файла окружения постфикса **_.example_** быть не должно.
...
* Распознавание на изображении колб и ограниченного списка цветов внутри них, в том числе на сложных фонах (когда задний фон практически сливается с контурами или даже цветами внутри колб, пример решения подобного уровня продемонстрирован в видео ниже).
...
[Пример использования бота для решения одного из уровней в игре на переливание жидкостей](https://www.youtube.com/shorts/yA5hJfQxB9k)
